...
func createStudent(c *gin.Context) {
	...
	if newStudent.Name == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "name is requird"})
		return
	}
...
func updateStudent(c *gin.Context) {
	...
	for i, student := range students {
		if student.ID == id {
			// ใช้ newStudent แทน updateStudent
			newStudent.ID = id // เก็บ ID เดิมไว้
...
func deleteStudents(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "student not found"})
}
...
func main() {
	...
	api := r.Group("/api/v1")
	{
		...
		api.DELETE("/students", deleteStudents)
	}
	r.Run(":8080")
