...
func getYear(c *gin.Context) {
	year := c.Param("year") // ดึงปีจาก URL
	books := []Book{}

	// query ข้อมูลจาก DB ตามปี
	rows, err := db.Query("SELECT id, title, author, isbn, year, price FROM books WHERE year = $1", year)
	if err != nil {
	...
	for rows.Next() {
		var book Book
		rows.Scan(&book.ID, &book.Title, &book.Author, &book.ISBN, &book.Year, &book.Price)
		books = append(books, book)
	}
...
func main() {
	...
	api := r.Group("/api/v1")
	{
		...
		api.GET("/books/year/:year", getYear)
